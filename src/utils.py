...
def _option_token_length(tokenizer, prompt, completed_prompt):
    """Return the answer span length while accounting for SentencePiece boundary retokenization."""
    prompt_ids = tokenizer.encode(prompt, add_special_tokens=True)
    full_ids = tokenizer.encode(completed_prompt, add_special_tokens=True)
    common = 0
    for prompt_id, full_id in zip(prompt_ids, full_ids):
        if prompt_id != full_id:
            break
        common += 1
    option_len = len(full_ids) - common
    if option_len <= 0:
        raise ValueError("Candidate answer produced no tokens; check the Llama prompt separator")
    return option_len
...
def forward_wrap_with_option_len_dpzero(self, input_ids=None, labels=None, option_len=None, num_options=None, return_dict=None, dpzero=False, **kwargs):
    ...
    # Padding is identified through attention_mask when available. Llama commonly
    # uses <unk> as padding, so masking by token id would hide genuine <unk> tokens.
...
def encode_prompt(task, template, train_samples, eval_sample, tokenizer, max_length, sfc=False, icl_sfc=False, generation=False, generation_with_gold=False, max_new_tokens=None):
    ...
    if not generation:
        ...
        option_lens = [
            _option_token_length(tokenizer, unverbalized_eval_prompt, verbalized_eval_prompt)
        ...
        if generation_with_gold:
            ...
            option_lens = [
                _option_token_length(tokenizer, unverbalized_eval_prompt, p)
                for p in verbalized_eval_prompts
            ]
            final_prompts = [(train_prompts + task.train_sep + eval_prompt).lstrip().strip(' ') for eval_prompt in verbalized_eval_prompts] 
        else:
            option_lens = [0]
    ...
    # Tokenize 
    encodings = [tokenizer.encode(final_prompt, add_special_tokens=True) for final_prompt in final_prompts]
    ...
    for encoding, option_len in zip(encodings, option_lens):
        if option_len >= len(encoding):
            raise ValueError(
                f"Answer span ({option_len} tokens) consumes the full retained prompt ({len(encoding)} tokens)"
            )
