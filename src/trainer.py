...
class OurTrainer(Trainer):
    ...
    def _inner_training_loop(
        ...
        if self.args.dpzero:
            ...
            if sample_rate > 1:
                raise ValueError(
                    f"DPZero batch size ({total_train_batch_size}) exceeds the training set size ({num_examples})"
                )
    ...
    def dpzero_clip(self, loss_diff, C=1.):
        return torch.clamp(loss_diff, min=-C, max=C)
    ...
    def dpzero_step(self, model, inputs):
        ...
        if not self.named_parameters_to_optim:
            raise ValueError("DPZero found no trainable parameters")
